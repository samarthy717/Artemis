...
namespace SupanthaPaul
{
	public class PlayerController : MonoBehaviour
	{
		...
		[Header("Dashing")]
		[SerializeField] private float dashSpeed = 29f;
		...
		private void FixedUpdate()
		{
			...
			if (isCurrentlyPlayable)
			{
				...
				if(m_wallJumping)
				{
					m_rb.velocity = Vector2.Lerp(m_rb.velocity, (new Vector2(moveInput.x * speed, m_rb.velocity.y)), 1.5f * Time.fixedDeltaTime);
				}
				else
				{
					if(canMove && !m_wallGrabbing)
						m_rb.velocity = new Vector2(moveInput.x * speed, m_rb.velocity.y);
					else if(!canMove)
				...
				// Flipping
				if (!m_facingRight && moveInput.x > 0f)
					Flip();
				else if (m_facingRight && moveInput.x < 0f)
					Flip();
                ...
                if (m_onWall && !isGrounded && m_rb.velocity.y <= 0f && m_playerSide == m_onWallSide)
				{
					...
					m_wallGrabbing = true;
					m_rb.velocity = new Vector2(moveInput.x * speed, -slideSpeed);
        ...
        void OnAttack()
        {
			...
			for(int i=0;i<hits.Length; i++)
			{
				...
				if (idamagable != null) {
					// damage
					idamagable.DamageAmount(AttackDamage);				
				}
			}
        }
        ...
        void OnJump(InputValue value)
        {
			...
			if (value.isPressed)
            {
                ...
                //JumpValue = context.ReadValue<Vector2>();
				
                // Your existing jump logic here
                ...
                else if (m_wallGrabbing && moveInput.x != m_onWallSide)        // wall jumping off the wall
                {
                    m_wallGrabbing = false;
                    m_wallJumping = true;
                ...
                else if (m_wallGrabbing && moveInput.x != 0 && (moveInput.x == m_onWallSide))      // wall climbing jump
                {
                    m_wallGrabbing = false;
                    m_wallJumping = true;
        ...
        public void Die()
		{
			if (IsAlive && m_rb.IsTouchingLayers(LayerMask.GetMask("Enemies", "Hazards")))
			{
				Debug.Log("Death");
				//myAnimator.SetBool("IsDead", true);
				IsAlive = false;
				m_rb.velocity = DeathKick;
			}
		}
	}
}
